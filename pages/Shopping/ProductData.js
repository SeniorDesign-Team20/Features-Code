// productData.js
import * as productInfo from "./../../files_to_modify/Products/product_info";
import product1Image from "./../../files_to_modify/Products/product1.png";
import product2Image from './../../files_to_modify/Products/product2.png';
import product3Image from './../../files_to_modify/Products/product3.png';
import product4Image from './../../files_to_modify/Products/product4.png';
import product5Image from './../../files_to_modify/Products/product5.png';
import product6Image from './../../files_to_modify/Products/product6.png';
import product7Image from './../../files_to_modify/Products/product7.png';
// ...

const products = [
  { 
    id: 1, 
    name: productInfo.name1, 
    price: productInfo.price1, 
    description: productInfo.description1, 
    image: product1Image, 
    categories: productInfo.categories1
  },
  { 
    id: 2, 
    name: productInfo.name2, 
    price: productInfo.price2, 
    description: productInfo.description2, 
    image: product2Image, 
    categories: productInfo.categories2
  },
  { 
    id: 3, 
    name: productInfo.name3, 
    price: productInfo.price3, 
    description: productInfo.description3, 
    image: product3Image, 
    categories: productInfo.categories3
  },
  { 
    id: 4, 
    name: productInfo.name4, 
    price: productInfo.price4, 
    description: productInfo.description4, 
    image: product4Image, 
    categories: productInfo.categories4
  },
  { 
    id: 5, 
    name: productInfo.name5, 
    price: productInfo.price5, 
    description: productInfo.description5, 
    image: product5Image, 
    categories: productInfo.categories5
  },
  { 
    id: 6, 
    name: productInfo.name6, 
    price: productInfo.price6, 
    description: productInfo.description6, 
    image: product6Image, 
    categories: productInfo.categories6
  },
  { 
    id: 7, 
    name: productInfo.name7, 
    price: productInfo.price7, 
    description: productInfo.description7, 
    image: product7Image, 
    categories: productInfo.categories7
  },
  { 
    id: 8, 
    name: productInfo.name8, 
    price: productInfo.price8, 
    description: productInfo.description8, 
    image: product7Image, 
    categories: productInfo.categories8
  },
  { 
    id: 9, 
    name: productInfo.name9, 
    price: productInfo.price9, 
    description: productInfo.description9, 
    image: product7Image, 
    categories: productInfo.categories9
  },
  { 
    id: 10, 
    name: productInfo.name10, 
    price: productInfo.price10, 
    description: productInfo.description10, 
    image: product7Image, 
    categories: productInfo.categories10
  },
  { 
    id: 11, 
    name: productInfo.name11, 
    price: productInfo.price11, 
    description: productInfo.description11, 
    image: product7Image, 
    categories: productInfo.categories11
  },
  { 
    id: 12, 
    name: productInfo.name12, 
    price: productInfo.price12, 
    description: productInfo.description12, 
    image: product7Image, 
    categories: productInfo.categories12
  },
  { 
    id: 13, 
    name: productInfo.name13, 
    price: productInfo.price13, 
    description: productInfo.description13, 
    image: product7Image, 
    categories: productInfo.categories13
  },
  { 
    id: 14, 
    name: productInfo.name14, 
    price: productInfo.price14, 
    description: productInfo.description14,
    image: product7Image,
    categories: productInfo.categories15
  }
];


export default products;
  